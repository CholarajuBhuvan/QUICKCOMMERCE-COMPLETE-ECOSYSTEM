...
const initialState = {
  ...
  isOpen: false,
  filters: {
    type: '', // 'order', 'system', 'user', 'inventory'
...
const notificationsSlice = createSlice({
  ...
  reducers: {
    ...
    updateSettings: (state, action) => {
      ...
      try {
        localStorage.setItem('admin_notification_settings', JSON.stringify(state.settings));
      } catch (error) {
...
const shouldPlaySound = (notification, settings) => {
  ...
  switch (notification.type) {
    ...
    case 'inventory':
      return settings.inventoryNotifications;
    case 'user':
      return settings.userNotifications;
    default:
      return true;
  }
};
...
const playNotificationSound = (priority) => {
  try {
    ...
    if ('AudioContext' in window || 'webkitAudioContext' in window) {
      ...
      const frequency = priority === 'urgent' ? 900 : priority === 'high' ? 700 : 500;
      const duration = priority === 'urgent' ? 0.4 : 0.2;
