...
const initialState = {
  ...
  isOpen: false,
  filters: {
    type: '', // 'order_available', 'stock_low', 'system_alert', etc.
...
const notificationsSlice = createSlice({
  ...
  reducers: {
    ...
    updateSettings: (state, action) => {
      ...
      try {
        localStorage.setItem('picker_notification_settings', JSON.stringify(state.settings));
      } catch (error) {
    ...
    loadSettings: (state) => {
      try {
        const savedSettings = localStorage.getItem('picker_notification_settings');
        if (savedSettings) {
...
const shouldPlaySound = (notification, settings) => {
  ...
  switch (notification.type) {
    ...
    case 'stock_low':
    case 'stock_out':
      return settings.stockNotifications;
    case 'system_alert':
...
const playNotificationSound = (priority) => {
  try {
    ...
    if ('AudioContext' in window || 'webkitAudioContext' in window) {
      ...
      const duration = priority === 'urgent' ? 0.3 : 0.2;
